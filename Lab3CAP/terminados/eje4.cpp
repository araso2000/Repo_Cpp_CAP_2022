...
int main(int argc, char* argv[]) {
    ...
    MPI_Scatter(&arraySend, (100 / world_size), MPI_INT, &arrayReceive, 100, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    if (world_rank != 0) {
        ...
        for (int ii = 0; ii < (100 / world_size); ii++) {
            arrayReceive[ii] = arrayReceive[ii] * 2;
        }
    }

    MPI_Gather(&arraySend, 100, MPI_INT, &arrayReceive, 100, MPI_INT, 0, MPI_COMM_WORLD);

    if (world_rank == 0) {
        cout << "Vector recogido" << endl;
        for (int ii = 0; ii < 100; ii++) {
            cout << arrayReceive[ii] << "  ";
        }
    }
